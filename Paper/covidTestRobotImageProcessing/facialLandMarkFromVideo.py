    ...
    for i, rect in enumerate(rects):
        ...
        if result > mouthOpenThreshold:
            ...
            if human_detect_flag:
                ...
                # Dynamic tracking: align mouth center with frame center and approach
                h, w = frame.shape[:2]
                mouth_center_x = int((shape[48][0] + shape[54][0]) / 2)
                mouth_center_y = int((shape[51][1] + shape[57][1]) / 2)
                dx = mouth_center_x - (w // 2)
                dy = mouth_center_y - (h // 2)

                cmd_parts = []
                # Horizontal adjustment (X motor): right if dx > deadband, left if dx < -deadband
                if dx > TRACKING_DEADBAND_PIXELS:
                    stepsX = int(min(MAX_STEP_PER_UPDATE, abs(dx * PIXELS_TO_STEPS_X)))
                    cmd_parts.append(f"XR{stepsX}")
                elif dx < -TRACKING_DEADBAND_PIXELS:
                    stepsX = int(min(MAX_STEP_PER_UPDATE, abs(dx * PIXELS_TO_STEPS_X)))
                    cmd_parts.append(f"XL{stepsX}")

                # Vertical adjustment (Y motor): up/top if dy < -deadband, back/down if dy > deadband
                if dy > TRACKING_DEADBAND_PIXELS:
                    stepsY = int(min(MAX_STEP_PER_UPDATE, abs(dy * PIXELS_TO_STEPS_Y)))
                    cmd_parts.append(f"YB{stepsY}")
                elif dy < -TRACKING_DEADBAND_PIXELS:
                    stepsY = int(min(MAX_STEP_PER_UPDATE, abs(dy * PIXELS_TO_STEPS_Y)))
                    cmd_parts.append(f"YT{stepsY}")

                # Gentle forward approach on Z while mouth remains open
                if mouth_open_flag % 3 == 0:  # rate-limit Z moves
                    cmd_parts.append(f"ZT{Z_APPROACH_STEPS_PER_ITER}")

                if cmd_parts:
                    serialCommunication.sendCmd("".join(cmd_parts) + "\n")
                    time.sleep(0.05)
        else:
