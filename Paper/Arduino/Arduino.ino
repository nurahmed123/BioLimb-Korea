...
void loop() {
  if (Serial.available() > 0) {
    ...
    static bool hasBuds = false;

    if (commandString == "AUTO") {
      autoMode = true;
      Serial.println("ACK_AUTO_ON");
    } else if (commandString == "MANUAL") {
      autoMode = false;
      Serial.println("ACK_AUTO_OFF");
    } else if (commandString == "HD") {  // human detected (proximity)
      if (autoMode && !hasBuds) {
        takeBuds();
        IndexPos();
        hasBuds = true;
        Serial.println("ACK_TB_IN");
      }
    } else if (commandString == "NHD") {  // not human detected
      // no-op in auto mode
    } else if (commandString == "MO") {  // mouth open
      if (autoMode && hasBuds) {
        TakeSample();
        PutBuds();
        hasBuds = false;
    ...
    } else if (commandString == "MC") {  // mouth closed
      // no-op in auto mode
    } else if (commandString == "TB") {  // take buds
      takeBuds();
      ...
      PutBuds();
    } else if (commandString == "FULL") {  // full workflow
      FullWorkOut();
    } else {
      // treat as low-level motion/servo command sequence
